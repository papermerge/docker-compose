# Papermerge + Microsoft Entra ID (OIDC)

This compose file runs Papermerge behind `oauth2-proxy`, using Microsoft Entra ID (Azure AD) as an OIDC provider.

- Papermerge (upstream) runs on the internal Docker network.
- `oauth2-proxy` listens on the host at http://localhost:8081 and forwards authenticated requests to Papermerge.

This is intended for local/dev/demo usage.

## Quick Start

1. Create a `.env` file (see `dot_env.example`).
2. Create an Entra ID App Registration (redirect URL must match the compose config).
3. Start containers: `docker compose up -d`
4. Open Papermerge at http://localhost:8081
5. After first login, optionally grant your user superuser in the DB (see below).
...
Copy `dot_env.example` to `.env` and fill in values:

- `ENTRA_TENANT_ID`: your tenant (directory) ID
- `ENTRA_CLIENT_ID`: the App Registration (client) ID
- `ENTRA_CLIENT_SECRET`: a client secret *value* created for the app
- `OAUTH2_COOKIE_SECRET`: 16/24/32 bytes secret (32 chars is fine for local)

### 2) Configure Entra ID App Registration

In Azure Portal → Microsoft Entra ID → App registrations:

1. Create a new registration (single tenant is fine for local use).
...
On first login, Papermerge creates a DB user record. This setup uses the OIDC `sub` claim as the user id, so the username stored in Papermerge will look like a long opaque identifier.
...
-- Pick your non-system user and grant superuser
update users set is_superuser=true where username='<your-username>';
```

## Notes

- `oauth2-proxy` is exposed on `localhost:8081`.
- Postgres is exposed on `localhost:5432` (adjust/remove if it conflicts with other stacks).
- For production, terminate TLS and set cookie secure settings appropriately.


# Complete HTTP Request Flow Diagram

## Papermerge + OAuth2-Proxy + Entra ID

NOTE: The diagram below was originally written for the Zitadel example and may still contain Zitadel-specific URLs/names. The important part for Entra ID is that the browser is redirected to `https://login.microsoftonline.com/<tenant>/...` and back to `http://localhost:8081/oauth2/callback`.
